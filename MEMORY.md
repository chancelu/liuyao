...
- **调用 Claude Code 必须加 `--dangerously-skip-permissions`**，每次都要，否则会卡住失败
...
- 2026-03-15：装卦显示修正（六亲手机端显示、六神标注、变卦纳支、摇卦概率改为真实三枚铜钱模拟）
- 2026-03-15：首页八卦圆盘改为文王后天八卦方位 + 金色漂浮粒子
- 2026-03-15：分享图改为完整长图（排盘+分析+网站链接）
- **支付方案**：确定用 Lemon Squeezy（MoR模式，个人身份注册，处理全球税务），只做海外用户
- **多语言**：计划用 i18n 支持 UI 多语言，LLM 分析由模型自行处理多语言
