...
export default function AdminPage() {
  ...
  async function handleGrant(e: React.FormEvent) {
    ...
    if (isNaN(num) || num <= 0) { setResult({ type: 'err', text: m.invalidAmount }); return; }
    ...
    try {
      ...
      if (data.success) {
        setResult({
          type: 'ok',
          text: m.granted
            .replace('{uid}', data.targetUid)
            .replace('{amount}', data.amount)
            .replace('{balance}', data.newBalance),
        });
      ...
      setBusy(false);
      setResult({ type: 'err', text: m.networkError });
    }
  }
  ...
  if (loading) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="animate-breathe text-sm text-[var(--text-dim)]">{m.verifying}</div>
      </div>
    );
  }
  ...
  return (
    <div className="glow-top mx-auto max-w-xl space-y-8">
      <div className="animate-fade-in-up">
        <div className="text-[10px] tracking-[0.4em] text-[var(--text-dim)] uppercase">{m.badge}</div>
        <h1 className="mt-2 font-display text-2xl font-extralight tracking-[0.02em] text-white">{m.title}</h1>
      ...
      <div className="animate-fade-in-up delay-100 card-solid rounded-2xl p-6 sm:p-8">
        <div className="mb-6 text-[10px] tracking-[0.25em] text-[var(--gold)] uppercase">{m.grantTitle}</div>

        <form onSubmit={handleGrant} className="space-y-4">
          <div className="space-y-2">
            <label className="block text-[10px] tracking-[0.25em] text-[var(--text-dim)] uppercase">{m.uidLabel}</label>
          ...
          <div className="space-y-2">
            <label className="block text-[10px] tracking-[0.25em] text-[var(--text-dim)] uppercase">{m.amountLabel}</label>
          ...
          <div className="space-y-2">
            <label className="block text-[10px] tracking-[0.25em] text-[var(--text-dim)] uppercase">{m.noteLabel}</label>
            <input type="text" placeholder={m.notePlaceholder}
