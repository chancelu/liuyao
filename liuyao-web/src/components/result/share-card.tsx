...
export function ShareCard({ result, onClose, accessToken }: { result: MockResult; onClose: () => void; accessToken?: string | null }) {
  ...
  }, [result.primaryHexagram, accessToken]);
        ...
        >
          ...
          <div style={{ padding: '28px 28px 24px' }}>
            ...
            <div style={{
              ...
              <div style={{ fontSize: 10, color: '#666', letterSpacing: 3, marginBottom: 6 }}>所问</div>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
              <div style={{
                ...
                <div style={{ fontSize: 10, color: '#666', letterSpacing: 2, marginBottom: 8 }}>本卦</div>
              ...
              <div style={{
                ...
                <div style={{ fontSize: 10, color: '#666', letterSpacing: 2, marginBottom: 8 }}>变卦</div>
            ...
            {chart && (
              <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
                {[
                  { label: '月建', value: chart.monthBranch },
                  { label: '日辰', value: `${chart.dayStem}${chart.dayBranch}` },
                  { label: '旬空', value: `${chart.xunkong[0]}${chart.xunkong[1]}` },
                  { label: '宫', value: `${chart.primary.palace}宫(${chart.primary.palaceElement})` },
                ].map((m) => (
                  <div key={m.label} style={{
                    ...
                    <div style={{ fontSize: 11, color: '#ccc' }}>{m.value}</div>
                  </div>
                ))}
              </div>
            )}

            {/* Chart: 本卦 + 变卦 side by side with full details */}
            {chart && (
              <div style={{ display: 'flex', gap: 8, marginBottom: 18 }}>
                {/* 本卦 */}
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 9, color: '#666', letterSpacing: 2, marginBottom: 8, textAlign: 'center' }}>
                    本卦 · {chart.primary.name}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {lines.map((line) => (
                      <div key={line.position} style={{
                        ...
                        <span style={{ fontSize: 9, color: '#ccc', width: 12, flexShrink: 0, textAlign: 'center' }}>{YAO_POS[line.position - 1]}</span>
                        ...
                        {line.isShi && <span style={{ fontSize: 8, color: '#B8A070', marginLeft: 'auto' }}>世</span>}
                        {line.isYing && <span style={{ fontSize: 8, color: '#6B8FBF', marginLeft: 'auto' }}>应</span>}
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {result.summary && (
              <div style={{
                ...
                <div style={{ fontSize: 10, color: '#B8A070', letterSpacing: 3, marginBottom: 8 }}>卦象提要</div>
            ...
            {result.isAI && result.plainAnalysis && (
              <div style={{
                ...
                <div style={{ fontSize: 10, color: '#B8A070', letterSpacing: 3, marginBottom: 8 }}>卦象解读</div>
            ...
            {result.isAI && result.professionalAnalysis && (
              <div style={{
                ...
                <div style={{ fontSize: 10, color: '#B8A070', letterSpacing: 3, marginBottom: 8 }}>断卦释义</div>
